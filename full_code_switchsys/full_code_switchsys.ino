...
const int tactileSwitchPin = 7; // Change this to your switch pin
...
void setup() {
  ...
  pinMode(tactileSwitchPin, INPUT); // Enable internal pull-up resistor
}
...
void loop() {
  ...
  if (buttonPushCounter % 2 == 0) {
    ...
    static float z_distance_remem = distance;
    static float azimuth_remem = currentAzimuth;
    static float Roll_remem = Roll;
    static float Pitch_remem = Pitch;
    // Z-axis notification
    if (abs(distance - z_distance_remem) > 40) {
      digitalWrite(LED_PIN3, HIGH);
      delay(1000);
      digitalWrite(LED_PIN3, LOW);
    }
    ...
    Serial.println(z_distance_remem);
    delay(250);
  }
}
