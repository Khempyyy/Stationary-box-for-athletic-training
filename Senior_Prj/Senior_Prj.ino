...
void loop() {
  ...
  if (digitalRead(tactileSwitchPin) == HIGH) { // Switch pressed
    z_distance_remem = distance;
    azimuth_remem = c;
    Roll_remem = Roll;
    Pitch_remem = Pitch;
        //Z-axis notification
    ...
    //Yaw-move
    if (abs(azimuth_remem - previousAzimuth) > 3) {
      digitalWrite(LED_PIN0, HIGH);
      delay(1000);
      digitalWrite(LED_PIN0, LOW);
    }
